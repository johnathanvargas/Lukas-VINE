/**
 * ScoutingLogsButton Component
 * 
 * A React component for submitting scouting log entries.
...
const ScoutingLogsButton = () => {
  ...
  const [pests, setPests] = useState([{ name: '', severity: '', count: '' }]);
  ...
  const handlePestsChange = (index, field, value) => {
    const newPests = [...pests];
    newPests[index][field] = value;
    setPests(newPests);
  };

  const addPestRow = () => {
    setPests([...pests, { name: '', severity: '', count: '' }]);
  };

  const removePestRow = (index) => {
    if (pests.length > 1) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Validate at least one pest observation
      const validPests = pests.filter(pest => pest.name.trim());
      if (validPests.length === 0) {
        throw new Error('Please add at least one pest/disease observation');
      }

      if (!user) {
        throw new Error('You must be signed in to submit a scouting log');
      }
      ...
      // Insert scouting log into database
      ...
      if (insertError) {
        ...
        throw new Error('Failed to submit scouting log. Please try again.');
      }
      ...
      if (selectedFiles.length > 0) {
        try {
          ...
          setError('Scouting log submitted but some photos failed to upload');
        }
      }
      ...
      setError(err.message || 'Failed to submit scouting log');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="scouting-logs-button-container">
      {!showForm ? (
        <button 
          onClick={() => setShowForm(true)}
          className="scouting-logs-button"
          style={{
            padding: '12px 24px',
        ...
        <div 
          className="scouting-logs-form-modal"
          style={{
            position: 'fixed',
            top: 0,
            left: 0,
        ...
        >
          <div 
            className="scouting-logs-form-content"
            style={{
          ...
          >
            ...
            <h2 style={{ marginTop: 0, color: '#fd7e14' }}>Submit Scouting Log</h2>

            {!user ? (
              <div style={{ textAlign: 'center', padding: '20px' }}>
                <p>Please sign in to submit a scouting log.</p>
                <button 
              ...
              <div style={{ 
                ...
                <h3 style={{ color: '#155724', marginTop: 0 }}>Scouting Log Submitted Successfully!</h3>
              ...
              <form onSubmit={handleSubmit}>
                    ...
                    <div key={index} style={{ 
                      ...
                      <div style={{ display: 'flex', gap: '10px', marginBottom: '8px' }}>
                        <input
                          ...
                          value={pest.name}
                          onChange={(e) => handlePestsChange(index, 'name', e.target.value)}
                          required
                          style={{
                            flex: 1,
                            padding: '6px',
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <select
                          value={pest.severity}
                          onChange={(e) => handlePestsChange(index, 'severity', e.target.value)}
                          style={{
                            flex: 1,
                            padding: '6px',
                            border: '1px solid #ccc',
                        ...
                        <input
                          type="text"
                          placeholder="Count/population (optional)"
                          value={pest.count}
                          onChange={(e) => handlePestsChange(index, 'count', e.target.value)}
                          style={{
                            flex: 1,
                            padding: '6px',
                            border: '1px solid #ccc',
                ...
                >
                  {loading ? 'Submitting...' : 'Submit Scouting Log'}
                </button>
              </form>
            )}
          </div>
...
export default ScoutingLogsButton;
